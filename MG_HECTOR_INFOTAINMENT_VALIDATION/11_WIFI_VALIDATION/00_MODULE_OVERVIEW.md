# Module Overview: WiFi Validation

## Purpose

This module turns `WiFi Validation` into a production validation work package. The focus is hotspot, client mode, roaming, throughput and interruption tests. The expected learner output is not only theoretical understanding, but the ability to run a bench-level validation activity, collect evidence, explain failures, and defend the result in an OEM release review.
