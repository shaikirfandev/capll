# Module Overview: IVI Features

## Purpose

This module turns `IVI Features` into a production validation work package. The focus is radio, media, phone, navigation, UI, profiles and notifications. The expected learner output is not only theoretical understanding, but the ability to run a bench-level validation activity, collect evidence, explain failures, and defend the result in an OEM release review.
