# Module Overview: Functional Safety

## Purpose

This module turns `Functional Safety` into a production validation work package. The focus is safety goals, camera availability, warnings and validation evidence. The expected learner output is not only theoretical understanding, but the ability to run a bench-level validation activity, collect evidence, explain failures, and defend the result in an OEM release review.
