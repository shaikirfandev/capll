# Module Overview: CarPlay Android Auto

## Purpose

This module turns `CarPlay Android Auto` into a production validation work package. The focus is projection protocol validation and recovery behavior. The expected learner output is not only theoretical understanding, but the ability to run a bench-level validation activity, collect evidence, explain failures, and defend the result in an OEM release review.
