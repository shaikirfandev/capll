# Module Overview: Android Automotive

## Purpose

This module turns `Android Automotive` into a production validation work package. The focus is AAOS stack, services, adb, HAL, apps and vehicle properties. The expected learner output is not only theoretical understanding, but the ability to run a bench-level validation activity, collect evidence, explain failures, and defend the result in an OEM release review.
