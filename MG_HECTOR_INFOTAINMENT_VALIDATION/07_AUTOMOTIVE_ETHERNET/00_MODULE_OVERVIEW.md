# Module Overview: Automotive Ethernet

## Purpose

This module turns `Automotive Ethernet` into a production validation work package. The focus is DoIP, SOME/IP, service discovery, VLANs and packet capture. The expected learner output is not only theoretical understanding, but the ability to run a bench-level validation activity, collect evidence, explain failures, and defend the result in an OEM release review.
