# Module Overview: Vehicle Signal Simulation

## Purpose

This module turns `Vehicle Signal Simulation` into a production validation work package. The focus is speed, gear, doors, power mode, lamps and network states. The expected learner output is not only theoretical understanding, but the ability to run a bench-level validation activity, collect evidence, explain failures, and defend the result in an OEM release review.
