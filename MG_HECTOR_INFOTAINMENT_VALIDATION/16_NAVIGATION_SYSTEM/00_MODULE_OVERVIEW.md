# Module Overview: Navigation System

## Purpose

This module turns `Navigation System` into a production validation work package. The focus is GNSS, map, route, guidance, dead reckoning and voice prompts. The expected learner output is not only theoretical understanding, but the ability to run a bench-level validation activity, collect evidence, explain failures, and defend the result in an OEM release review.
